...
def dejar_solo_cifras(txt):
    "función que elimina caracteres y deja sólo los dígitos"
  return "".join(c for c in txt if c.isdigit())
#Convierte la columna de un dataframe en número:
df["% Goals-to-shots"] = df["% Goals-to-shots"].map(dejar_solo_cifras).astype(int)

#Otra forma con pandas:
# observamos qué columnas tienen %
col_to_change = [col for col in df.columns if df[col].astype(str).str.contains('%').any()]
# modificamos la columna donde convertimos todos los datos a float eliminado el string:
